...
int main(int argc, char *argv[])
{
    ...
    if(true)
    {
        ifstream infile("./data/block2.txt");
    ...
    myaddr.sin_port = htons(Ports::serverB);
    ...
    cout << "The ServerB is up and running using UDP on port " << ntohs(myaddr.sin_port) << endl;
    ...
    while(1)
    {
        ...
        } else if(requests[0].compare("make_transaction") == 0)
        {
            ...
            ofstream out;
            out.open("./data/block2.txt", ios::app);
            string to_write = "\n";
            to_write += requests[4];
            to_write += " ";
            to_write += requests[1];
            ...
            // DEBUG MSG
            // cout << "DEBUG: wrote " << to_write << " into block2" << endl;
            // END DEBUG
